...
- Server state (API responses, caching, fetching/loading/error) goes through TanStack Query (`@tanstack/vue-query`) via `useQuery`/`useMutation`, each wrapped in its own composable under `src/composables/` (`useXxxQuery.ts`) rather than called inline in components. No view calls the API yet (the backend has no routes), so there's no worked example in the codebase — follow this shape when the first one is added. After a mutation, prefer `queryClient.invalidateQueries` over manually patching local arrays to stay in sync. Never store API response data in a Pinia store.
- Pinia (`src/stores`, one store per domain) is for global UI state only — auth info, theme, notifications, and other state shared app-wide that isn't server data. Define stores with the setup-function style, not the Options style, and don't call `fetch`/`axios` from inside a Pinia action; that's TanStack Query's job. `src/stores/notification.ts` is a worked reference: `UploaderView.vue` calls it directly for validation and confirmation messages (e.g. after copying the join code) — the simplest way to see the store work.
...
- API client (`src/api/client.ts`): no test currently, since the backend has no routes to smoke-test typed RPC methods against — add `client.test.ts` back once one exists.
